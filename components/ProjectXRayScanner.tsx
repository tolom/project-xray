...
// ==================== Main Component ====================
type ProjectXRayScannerProps = {
  onActivityChange?: (isActive: boolean) => void;
  showInputIntro?: boolean;
  showChrome?: boolean;
  showExamples?: boolean;
  resultHeaderTopClassName?: string;
};

export default function ProjectXRayScanner({
  onActivityChange,
  showInputIntro = true,
  showChrome = true,
  showExamples = true,
  resultHeaderTopClassName = "top-16",
}: ProjectXRayScannerProps) {
...
Deep-scanned files: **${visibleResult.criticalFilesFetched}/${visibleResult.scannedFiles}**\n\n`;
  ...
  // ==================== Render ====================
  return (
    <div className="bg-[#0a0a0a] text-white selection:bg-cyan-500/30">
      {showChrome && (
      <div className="mx-auto max-w-7xl px-5 pt-4 sm:px-6">
        <div className="flex flex-col gap-3 rounded-2xl border border-white/10 bg-white/[0.03] p-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-wrap items-center gap-2 text-xs text-white/55">
            <Badge variant="outline" className="border-cyan-400/30 bg-cyan-400/10 text-cyan-200">MVP 3.1</Badge>
            <span>100% client-side analysis</span>
            <span className="hidden sm:inline">•</span>
            <span>source code never leaves the device</span>
          </div>
          ...
          {isSignedIn ? (
            <div className="flex flex-wrap items-center gap-2">
              <button
                onClick={() => setShowGitHubHelp(true)}
                className="flex cursor-pointer items-center gap-1.5 rounded-full border border-white/15 bg-white/5 px-2.5 py-1 text-xs text-white/70 transition-colors hover:bg-white/10 hover:text-white"
                title="Configure GitHub access"
              >
                <div className={`h-1.5 w-1.5 rounded-full ${githubToken ? 'bg-emerald-500' : 'bg-amber-500'}`} />
                <span>{githubToken ? "Private repository access enabled" : "Public only"}</span>
              </button>
              <Button variant="ghost" size="sm" onClick={() => signOut()} className="gap-2 hover:bg-white/10 hover:text-white">
                <LogOut className="w-4 h-4" /> Sign out
              </Button>
              <Button variant="ghost" size="sm" onClick={rescan} disabled={!result || isScanning} className="gap-2 hover:bg-white/10 hover:text-white">
      ...
      {/* Input zone, shown only before the first scan. */}
      {!result && (
        <div className={`max-w-4xl mx-auto px-5 text-center sm:px-6 ${showInputIntro ? "pt-6 pb-12" : "pt-0 pb-0"}`}>
          {showInputIntro && (
            <>
              <h2 className="text-3xl font-semibold leading-tight tracking-tight sm:text-5xl">
                Scan repository
              </h2>
              <p className="mx-auto mb-8 mt-3 max-w-xl text-base leading-relaxed text-white/60 sm:text-lg">
                Paste a public GitHub URL or sign in with GitHub for a private repository.
              </p>
            </>
          )}
          ...
          <div className="flex flex-col gap-3 max-w-2xl mx-auto">
            <div className="flex flex-col gap-3 sm:flex-row">
              <Input
                ...
                className="h-14 flex-1 border-white/15 bg-zinc-950 text-base placeholder:text-white/30 focus:border-white/40 sm:text-lg"
                disabled={isScanning}
            ...
            <div className="flex flex-wrap gap-2 justify-center text-xs text-white/50">
              ...
              {EXAMPLE_REPOS.map((ex, i) => (
                <button
                  ...
                  className="cursor-pointer hover:text-white underline-offset-2 hover:underline transition-colors"
                >
      ...
      {result && (
        <div className={`border-b border-white/10 bg-[#0a0a0a]/95 backdrop-blur sticky ${resultHeaderTopClassName} z-40`}>
      ...
      {showGitHubHelp && (
        <div className="fixed inset-0 z-[100] overflow-y-auto bg-black/80 p-4" onClick={() => setShowGitHubHelp(false)}>
          <div className="mx-auto flex min-h-full w-full max-w-xl items-center py-4" onClick={e => e.stopPropagation()}>
            <GitHubAuthHelp 
